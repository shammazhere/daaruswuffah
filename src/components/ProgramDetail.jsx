...
const ProgramDetail = () => {
    ...
    if (!program) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-peach py-20">
                <h2 className="text-3xl font-serif font-bold text-navy mb-4">Program Not Found</h2>
    ...
    return (
        <div className="bg-peach min-h-screen flex flex-col">
            {/* Hero Header - Full Cover with Background Image */}
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10 w-full">
                    ...
                    >
                        <div className="inline-flex items-center gap-4 px-6 py-2 rounded-full bg-gold/10 border border-gold/20 mb-8 text-gold backdrop-blur-md">
                            <span className="text-shadow-sm">{getIcon(program.iconName)}</span>
            ...
            <div className="flex-grow max-w-7xl mx-auto py-20 px-6 lg:px-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                    ...
                    <motion.div
                        ...
                        viewport={{ once: true }}
                    >
                        <div className="bg-white rounded-3xl p-10 shadow-premium border border-navy/5">
                            <h3 className="text-2xl font-serif font-bold text-navy mb-8 border-b border-navy/5 pb-4">Course Overview</h3>
                            <p className="text-dark/70 leading-relaxed text-lg mb-6">
                                {program.description}
                            </p>
                            <p className="text-dark/70 leading-relaxed text-lg italic">
                                At AS-SWUFFAH, we believe in providing education that transcends mere academics.
                                The {program.name} program is meticulously crafted to ensure that every student
                                receives the guidance, knowledge, and spiritual nurturing required to excel.
                            </p>
                        </div>

                        <div className="bg-white rounded-3xl p-10 shadow-premium border border-navy/5">
                            <h3 className="text-2xl font-serif font-bold text-navy mb-8">What You Will Gain</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {program.features.map((feature, idx) => (
                                    <div key={idx} className="flex items-center gap-4 p-5 bg-peach/30 rounded-2xl group hover:bg-gold/10 transition-colors">
                                        <div className="w-8 h-8 rounded-full bg-gold/20 flex items-center justify-center text-gold shrink-0">
                                            <CheckCircle size={16} />
                                        </div>
                                        <span className="font-bold text-navy text-sm uppercase tracking-wide">{feature}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    >
                        <div className="bg-navy rounded-3xl p-10 sticky top-32 shadow-premium">
                            ...
                            <div className="space-y-8">
                                ...
                                <Link
                                    to="/contact"
                                    className="w-full btn-gold py-5 mt-6 uppercase tracking-[0.2em] text-sm flex items-center justify-center text-center"
                                >
                                    Apply Now
                                </Link>
